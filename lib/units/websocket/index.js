    ...
    io.on('connection', function(socket) {
        ...
        new Promise(function(resolve) {
            ...
            socket.on('disconnect', resolve)
                ...
                // TODO: Support the reverse response
                ...
                .on('device.install', function(channel, responseChannel, data) {
                    ...
                    push.send([
                        channel
                        , wireutil.transaction(responseChannel, new wire.InstallMessage(data.href, data.launch === true, isApi, JSON.stringify(data.manifest), installFlags))
                    ])
                })
                .on('device.installIos', function(channel, responseChannel, data) {
                    ...
                    push.send([
                        channel
                        , wireutil.transaction(responseChannel, new wire.InstallMessage(data.href, data.launch === true, JSON.stringify(data.manifest)))
                    ])
                })
                ...
                // #455 and #459
                    ...
                    push.send([
                        channel
                        , wireutil.transaction(responseChannel, new wire.LogcatStartMessage({filters: {tag: '*', priority: 2}}))
                    ])
                })
                .on('logcat.startIos', function(channel, responseChannel, data) {
                    ...
                    push.send([
                        channel
                        , wireutil.transaction(responseChannel, new wire.LogcatStartMessage({filters: {tag: data.filters[0], priority: 1}}))
                    ])
                })
